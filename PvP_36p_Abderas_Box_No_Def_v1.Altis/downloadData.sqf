/*
 =======================================================================================================================
	...
	Author:		T-800a / cuel for percentages. Modified by Hellfire for CNTO PvP mission.
 ...
 ===== LICENSE =========================================================================================================
	...
	1. Hellfire: And that's exactly what I did !

 =======================================================================================================================
*/

T8L_var_fileSize = 100000;  								// Filesize ... smaller files will take shorter time to download!

T8L_var_TLine01 = "Download has been cancelled !";				// download aborted
T8L_var_TLine02 = "Download already in progress !";			// download already in progress by someone else
T8L_var_TLine03 = "Download has started !";				// download started
T8L_var_TLine04 = "Download finished !";			// download finished
T8L_var_TLine05 = "##  Start data transmission now ##";				// line for the addaction
...
T8L_fnc_abortActionLaptop =
{
	if ( T8L_var_DialogSucce ) then 
	{
		...
		T8L_var_DialogSucce = false; 
		"VictoryArea" setMarkerAlpha 0;
		["Download has been cancelled !", "hint", true, true] spawn BIS_fnc_MP;
	};
};
...
{
	private [ "_laptop", "_caller", "_id", "_tmpmarkerstr", "_markerstr"  ];
	_laptop = _this select 0;
	_caller = _this select 1;
	_id = _this select 2;
	
	//create that marker which will be used to show a rough area where the NAS can be
	_randomCenter = [ _laptop, 30] call hellf_fnc_getRandomPosInCircleAroundObject;
	_markerstr = createMarker ["VictoryArea", _randomCenter]; 
	...
	["Somebody started downloading the data. Check your map for an approximate location!", "hint", true, true] spawn BIS_fnc_MP;
	//let's make an area appear around where the download is occuring.
	"VictoryArea" setMarkerAlpha 0.5;
		...
		while { !T8L_var_DialogAbort } do
		{
			...
			if ( _newFile > T8L_var_FileSize ) then 
			{
				...
				if (side player == west) then {
					_side = "BluFor has won !";
				} else {
					_side = "OpFor has won !";
				};
				[format ["Download has finished ! %1", _side] , "hint", true, true] spawn BIS_fnc_MP;
				sleep 0.5;
				if (side player == west) then {
					[ ["BluForWin"], "BIS_fnc_endMission", west, true, true] call BIS_fnc_MP;
				    [ ["OpForLose",false], "BIS_fnc_endMission", east, true, true] call BIS_fnc_MP;
			
				} else {
					[ ["OpForWin"], "BIS_fnc_endMission", east, true, true] call BIS_fnc_MP;
				    [ ["BluForLose",false], "BIS_fnc_endMission", west, true, true] call BIS_fnc_MP;
				};
			};
